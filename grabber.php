...
if($query->num_rows > 0)
{
	while($row = $query->fetch_assoc())
	{
		...
		$data .= '/opt/add_vpn_user.sh "'.$username.'" "'.$password.'" 1>>/dev/null 2>>/dev/null;'.PHP_EOL;
	}
}
$location = '/opt/.process_accounts.sh';
$fp = fopen($location, 'w');
ftruncate($fp, 0);
...
if($query2->num_rows > 0)
{
	while($row2 = $query2->fetch_assoc())
	{
		...
		$data2 .= '/opt/del_vpn_user.sh "'.$toadd.'" 1>>/dev/null 2>>/dev/null'.PHP_EOL;
	}
}
